...
**iso20022** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `iso20022` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
iso20022 ... --format json | cognis-connect emit --to sigma   # Sigma rules
iso20022 ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
iso20022 ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
iso20022 ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `iso20022` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="iso20022") for rec in records]   # records = your JSON output
