# Comprehensive installer for cognis-digital/iso20022 (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# iso20022 is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "iso20022"
$Url  = "git+https://github.com/cognis-digital/iso20022.git"
$Git  = "https://github.com/cognis-digital/iso20022.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: iso20022"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: iso20022"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: iso20022"; exit 0 }
}
